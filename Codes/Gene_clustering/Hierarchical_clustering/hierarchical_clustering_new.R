...
setwd("C:/Users/asia/Desktop/Mtb_CGIP/Mtb_CGIP_Pred-main")
...
ecoli <- godata(annoDb = 'org.EcK12.eg.db', keytype = "SYMBOL", ont="BP", computeIC=FALSE)
...
pdf("Gene clustering_Result.pdf", 45, 12)
par(mar = c(5, 7, 5, 1), mgp = c(4, 1, 0))
plot(hc, main = "genes clustering",
     sub="", xlab="",
     cex.main = 4.0,
     cex.lab = 3.5,
     cex.axis = 3,
...
pdf("Gene hierarchical clustering dendrogram_Result.pdf", 45, 12)
par(mar = c(5, 7, 5, 1), mgp = c(4, 1, 0))
...
#sizeGrWindow(45, 12)
pdf("Gene_Dendrogram_Result.pdf", 45, 12)
plotDendroAndColors(marAll = c(5, 8, 3, 1),
                    dendro = hc,
...
png("BPclusters.png", width = 4800, height = 4000, res = 300)
#sizeGrWindow(40,8)
par(cex.main=1)                   # adjust font size of titles
heatmap.2(sim, main = 'Hierarchical clustering on Gene similarity matrix',
          ...
          col = color.scheme,         # color pattern of the heatmap
          trace="none",              # hide trace
          cexRow=0.2, cexCol = 0.2,      # size of row / column labels
          xlab = "genes", ylab = "genes",
